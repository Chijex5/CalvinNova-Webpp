...
export const useChatStore = create<ChatState>()(devtools((set, get) => ({
  ...
  getChatDetails: async (channelId: string) => {
    try {
      ...
      channel.on('message.updated', event => {
        if (event.message) {
          set(state => ({
            messages: state.messages.map(msg => msg.id === event.message!.id ? event.message! : msg)
          }));
        }
      });
      ...
      set({
        error: error instanceof Error ? error.message : 'Failed to fetch chat details'
      });
    } finally {
      set({
        isLoadingChats: false
  ...
  getAllChats: async () => {
    try {
      ...
      set({
        error: error instanceof Error ? error.message : 'Failed to fetch all chats'
      });
    } finally {
      set({
        isLoadingChats: false
  ...
  flagChat: async (channelId: string) => {
    try {
      ...
      // Update local state
      set(state => ({
        chats: state.chats.map(chat => chat.id === channelId ? {
          ...chat,
          data: {
            ...chat.data,
            is_spam: true
      ...
      set({
        error: error instanceof Error ? error.message : 'Failed to flag chat'
      });
    }
  },
...
export const useTypingUsers = (channelId: string): string[] => {
  return useChatStore(state => {
    ...
    return Object.keys(typingUsers).filter(id => typingUsers[id] && id !== userId);
  });
};
