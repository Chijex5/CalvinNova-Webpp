...
export const useAdminDataStore = create<AdminDataStore>()(devtools(persist((set, get) => ({
  // Initial state
  users: [],
  loading: false,
  error: null,
  ...
  updateUser: (id, updates) => {
    ...
    const updatedUsers = currentUsers.map(user => user.id === id ? {
      ...user,
      ...updates
    } : user);
    set({
  ...
  searchUsers: query => get().users.filter(user => user.name.toLowerCase().includes(query.toLowerCase())),
  filterUsersByRole: role => get().users.filter(user => user.role === role),
