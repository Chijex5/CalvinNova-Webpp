...
const ModernItemListingForm = () => {
  ...
  const categories = ['Electronics', 'Books', 'Clothing', 'Sports', 'Home & Garden', 'Beauty', 'Automotive', 'Toys', 'Music', 'Other'];
  ...
  const handleMultipleImageUpload = async (files: FileList) => {
        ...
        const response = await imageApi.post('/api/seller/upload', uploadData, {
          onUploadProgress: progressEvent => {
            if (progressEvent.total) {
              ...
              setUploadingImages(prev => prev.map(state => state.uploadId === uploadId ? {
                ...state,
    ...
    const successfulUploads = uploadResults.filter(result => result.success).map(result => ({
      id: result.id!,
      url: result.url!
    }));
    ...
    setTimeout(() => {
      setUploadingImages(prev => prev.filter(state => !initialUploadStates.some(initial => initial.uploadId === state.uploadId)));
    }, 2000);
  ...
  const ToggleButton = ({
    ...
    label: string;
      ...
      <div className="flex rounded-lg border border-gray-200 p-1 dark:bg-gray-900 bg-gray-50">
        {options.map(option => <button key={option.value} type="button" onClick={() => onChange(option.value)} className={`flex-1 px-4 py-2 rounded-md text-sm font-medium transition-all ${selected === option.value ? 'bg-blue-500 dark:bg-blue-400 text-white shadow-sm' : 'text-gray-600 dark:text-gray-400 dark:hover:text-gray-600 hover:text-gray-800'}`}>
            {option.label}
          </button>)}
      </div>
    </div>;
  const CategoryGrid: React.FC<{
    ...
    onChange
      ...
      {categories.map(category => <button key={category} type="button" onClick={() => onChange(category)} className={`p-3 rounded-lg border-2 transition-all ${selected === category ? 'border-blue-500 dark:border-blue-400 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300' : 'border-gray-200 dark:border-gray-600 hover:border-gray-300 dark:hover:border-gray-500 bg-white dark:bg-gray-800 text-gray-900 dark:text-white'}`}>
  ...
  const InfoBox = ({
    ...
    return <div className={`p-4 rounded-lg border ${styles[type]}`}>
      ...
      case 1:
        return <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {/* Existing images */}
              {formData.images.map((image, index) => <div key={index} className="relative aspect-square rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-800">
                  <img src={image.url} alt={`Upload ${index + 1}`} className="w-full h-full object-cover" />
                  <button type="button" onClick={() => removeImage(index)} className="absolute top-2 right-2 bg-red-500 dark:bg-red-600 text-white rounded-full p-1 shadow-md hover:bg-red-600 dark:hover:bg-red-700 transition-colors">
                    <X className="h-4 w-4" />
                  </button>
                </div>)}
              
              {/* Uploading images */}
              {uploadingImages.map((uploadState, index) => <div key={`uploading-${index}`} className="relative aspect-square rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-800 border-2 border-dashed border-gray-300 dark:border-gray-600">
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    {uploadState.status === 'uploading' && <>
                        ...
                        <div className="w-3/4 bg-gray-200 dark:bg-gray-700 rounded-full h-1">
                          <div className="bg-blue-500 dark:bg-blue-400 h-1 rounded-full transition-all duration-300" style={{
                    ...
                    {uploadState.status === 'error' && <>
                        ...
                        <button onClick={() => removeUploadingImage(uploadState.file)} className="text-xs text-red-500 dark:text-red-400 underline">
                          Remove
                        </button>
                      </>}
                  </div>
              ...
              {/* Upload button */}
              {formData.images.length + uploadingImages.filter(s => s.status === 'uploading').length < 4 && <label className="aspect-square border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg flex flex-col items-center justify-center cursor-pointer hover:border-gray-400 dark:hover:border-gray-500 transition-colors">
                  ...
                  <input type="file" accept="image/*" multiple onChange={e => e.target.files && handleMultipleImageUpload(e.target.files)} className="hidden" disabled={uploadingImages.some(state => state.status === 'uploading')} />
                </label>}
            </div>
            
            <div className="text-center text-sm text-gray-500 dark:text-gray-400">
              {formData.images.length} / 4 photos uploaded
              {uploadingImages.length > 0 && <span className="ml-2 text-blue-500 dark:text-blue-400">
        ...
        return <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={formData.title} onChange={e => handleInputChange('title', e.target.value)} className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent placeholder-gray-500 dark:placeholder-gray-400" placeholder="e.g., iPhone 13 Pro Max 128GB - Excellent condition" />
                {errors.title && <p className="text-red-500 dark:text-red-400 text-sm mt-1">{errors.title}</p>}
              </div>
              
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Description</label>
                <textarea value={formData.description} onChange={e => handleInputChange('description', e.target.value)} rows={4} className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent placeholder-gray-500 dark:placeholder-gray-400" placeholder="Describe your item in detail... Include brand, model, condition, any accessories, etc." />
                {errors.description && <p className="text-red-500 dark:text-red-400 text-sm mt-1">{errors.description}</p>}
              </div>
              
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Category</label>
                <CategoryGrid categories={categories} selected={formData.category} onChange={category => handleInputChange('category', category)} />
                {errors.category && <p className="text-red-500 dark:text-red-400 text-sm mt-1">{errors.category}</p>}
              </div>
              
              <ToggleButton label="Condition" options={[{
              value: 'new',
              label: 'New'
            }, {
              value: 'used',
              label: 'Used'
            }]} selected={formData.condition} onChange={condition => handleInputChange('condition', condition)} />
              
              <ToggleButton label="School" options={[{
              value: 'UNN',
              label: 'UNN'
            }, {
              value: 'UNEC',
              label: 'UNEC'
            }]} selected={formData.school} onChange={school => handleInputChange('school', school)} />
            </div>
          </div>;
      case 3:
        return <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="number" value={formData.price} onChange={e => handlePriceChange(e.target.value)} className="w-full pl-8 pr-4 py-4 text-lg rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent placeholder-gray-500 dark:placeholder-gray-400" placeholder="0" />
                </div>
                {errors.price && <p className="text-red-500 dark:text-red-400 text-sm mt-1">{errors.price}</p>}
              </div>
              
              {formData.price && Number(formData.price) > 0 && <InfoBox title="💰 Your Payout Breakdown" type="success">
                  <div className="space-y-2">
        ...
        return <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
              <div className="grid grid-cols-2 gap-4 mb-4">
                {formData.images.slice(0, 2).map((image, index) => <img key={index} src={image.url} alt={`Preview ${index + 1}`} className="w-full h-32 object-cover rounded-lg" />)}
              </div>
      ...
      <div className="max-w-md mx-auto bg-white dark:bg-gray-800 min-h-screen lg:max-w-2xl lg:my-8 lg:rounded-lg lg:shadow-xl dark:lg:shadow-2xl lg:min-h-0">
        ...
        <div className="sticky top-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 py-3 z-10 lg:rounded-t-lg">
          <div className="flex items-center justify-between">
            <button onClick={prevStep} disabled={currentStep === 1} className={`p-2 rounded-full ${currentStep === 1 ? 'text-gray-400 dark:text-gray-600' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>
          ...
          <div className="mt-4">
            <div className="flex justify-between items-center mb-2">
              ...
              return <div key={step.id} className="flex flex-col items-center">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center ${currentStep >= step.id ? 'bg-blue-500 dark:bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400'}`}>
            ...
            <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
              <div className="bg-blue-500 dark:bg-blue-600 h-2 rounded-full transition-all duration-300" style={{
        ...
        <div className="sticky bottom-0 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 p-4 lg:rounded-b-lg">
          <div className="max-w-md mx-auto lg:max-w-none">
            {currentStep < 4 ? <button onClick={nextStep} disabled={uploadingImages.some(state => state.status === 'uploading')} className={`w-full py-3 rounded-lg font-medium transition-colors flex items-center justify-center ${uploadingImages.some(state => state.status === 'uploading') ? 'bg-gray-400 dark:bg-gray-600 cursor-not-allowed text-white' : 'bg-blue-500 dark:bg-blue-600 text-white hover:bg-blue-600 dark:hover:bg-blue-700'}`}>
              ...
              </button> : <button onClick={handleSubmit} disabled={loading} className={`w-full py-3 rounded-lg font-medium transition-colors flex items-center justify-center
                  ${loading ? 'bg-gray-400 dark:bg-gray-600 cursor-not-allowed' : 'bg-green-500 dark:bg-green-600 hover:bg-green-600 dark:hover:bg-green-700 text-white'}
                `} aria-busy={loading}>
                {loading ? <>
