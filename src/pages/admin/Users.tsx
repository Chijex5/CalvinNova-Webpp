...
const ListingsSkeleton = () => <div className="flex items-center space-x-2">
...
const AdminUsersPage = () => {
  ...
  const [chatLengthLoading, setChatLengthLoading] = useState<{
    [key: string]: boolean;
  }>({});
  ...
  const [chatLengths, setChatLengths] = useState<{
  ...
  const [chatDataFetched, setChatDataFetched] = useState<{
    [key: string]: boolean;
  }>({});
  const [listingsDataFetched, setListingsDataFetched] = useState<{
    [key: string]: boolean;
  }>({});
  ...
  const getUserChatList = async (targetUserId: string) => {
    ...
    try {
      ...
      setChatLengths(prev => ({
        ...prev,
        [targetUserId]: totalChannels
      }));
      setChatDataFetched(prev => ({
        ...prev,
        [targetUserId]: true
      }));
    } catch (error) {
  ...
  const getUserListingsCount = async (targetUserId: string) => {
    ...
    try {
      ...
      setListingsCounts(prev => ({
        ...prev,
        [targetUserId]: totalListings
      }));
      setListingsDataFetched(prev => ({
        ...prev,
        [targetUserId]: true
      }));
    } catch (error) {
  ...
  const renderChatCount = (userId: string) => {
    ...
    if (count === 'error') {
      return <div className="flex items-center space-x-1 text-red-500 dark:text-red-400">
    ...
    return <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
        <div className="flex flex-col items-center space-y-4">
          <div className="flex space-x-1">
            ...
            <div className="w-2 h-2 bg-purple-500 dark:bg-purple-400 rounded-full animate-pulse" style={{
            animationDelay: '0.2s'
          }}></div>
            <div className="w-2 h-2 bg-purple-500 dark:bg-purple-400 rounded-full animate-pulse" style={{
            animationDelay: '0.4s'
  ...
  return <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="sticky top-0 z-50 bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center space-x-3">
            ...
            {users.filter(u => u.isFlagged).length > 0 && <div className="w-2 h-2 bg-red-500 rounded-full"></div>}
          </div>
          <div className="flex items-center space-x-2">
            <button onClick={handleRefresh} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors" disabled={loading}>
              <RefreshCw className={`w-5 h-5 text-gray-600 dark:text-gray-400 ${loading ? 'animate-spin' : ''}`} />
            </button>
            <button onClick={() => setShowFilters(!showFilters)} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">
      ...
      <div className="px-4 py-3 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
        <div className="relative">
          ...
          <input type="text" placeholder="Search by name, email, or ID..." className="w-full pl-10 pr-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
        </div>
      </div>

      {/* Filters Panel */}
      {showFilters && <div className="px-4 py-4 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Status</label>
              <select className="w-full p-2.5 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white" value={filters.status} onChange={e => setFilters({
            ...filters,
            ...
            <div>
              ...
              <input type="text" placeholder="Filter by campus" className="w-full p-2.5 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400" value={filters.campus} onChange={e => setFilters({
            ...filters,
          ...
          <div className="flex items-center">
            <input type="checkbox" id="verified" className="w-4 h-4 text-purple-600 border-gray-300 dark:border-gray-600 rounded focus:ring-purple-500 bg-white dark:bg-gray-700" checked={filters.verified} onChange={e => setFilters({
          ...filters,
      ...
      {/* Error Message */}
      {error && <div className="mx-4 mt-4 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
          <div className="flex items-center">
      ...
      <div className="px-4 py-2 space-y-3">
        ...
        return <div key={user.id} className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center space-x-3">
                  <div className="relative">
                    <div className="w-12 h-12 bg-purple-100 dark:bg-purple-900/30 rounded-full flex items-center justify-center">
                      {user.avatarUrl ? <img src={user.avatarUrl} alt={user.name} className="w-12 h-12 rounded-full object-cover" /> : <User className="w-6 h-6 text-purple-600 dark:text-purple-400" />}
                    </div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button onClick={() => toggleUserExpanded(user.id)} className="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded transition-colors">
              ...
              {/* Expanded Content */}
              {isExpanded && <div className="border-t border-gray-200 dark:border-gray-700 pt-3 mt-3 space-y-3">
                  <div className="grid grid-cols-2 gap-4">
              ...
              {user.isFlagged && <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-2 mb-3">
              ...
              <div className="flex flex-wrap gap-2">
                <button onClick={() => handleUserAction('view', user)} className="flex items-center space-x-1 px-3 py-1.5 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 rounded-lg text-sm font-medium hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-colors">
                ...
                {!user.isVerified && <button onClick={() => handleUserAction('verify', user)} className="flex items-center space-x-1 px-3 py-1.5 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 rounded-lg text-sm font-medium hover:bg-green-100 dark:hover:bg-green-900/30 transition-colors">
      ...
      {/* User Detail Modal */}
      {showUserDetail && selectedUser && <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-end sm:items-center justify-center p-4">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="text-center">
                <div className="w-20 h-20 bg-purple-100 dark:bg-purple-900/30 rounded-full mx-auto mb-4 flex items-center justify-center">
                  {selectedUser.avatarUrl ? <img src={selectedUser.avatarUrl} alt={selectedUser.name} className="w-20 h-20 rounded-full object-cover" /> : <User className="w-10 h-10 text-purple-600 dark:text-purple-400" />}
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center">
                    {selectedUser.isVerified ? <CheckCircle className="w-5 h-5 text-green-500" /> : <X className="w-5 h-5 text-red-500" />}
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 text-center">
                    {listingsLoading[selectedUser.id] ? <div className="flex flex-col items-center">
                      ...
                      </div> : listingsCounts[selectedUser.id] === 'error' ? <div className="flex flex-col items-center">
                  ...
                  <div className="bg-green-50 dark:bg-green-900/20 rounded-lg p-4 text-center">
                    {chatLengthLoading[selectedUser.id] ? <div className="flex flex-col items-center">
                      ...
                      </div> : chatLengths[selectedUser.id] === 'error' ? <div className="flex flex-col items-center">
                        <AlertCircle className="w-6 h-6 text-red-500 mb-2" />
                        <div className="text-xs text-red-700 dark:text-red-400">Error</div>
                      </div> : <>
              ...
              {selectedUser.isFlagged && <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
      ...
      {/* Warning Modal */}
      {showWarningModal && selectedUser && <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-end sm:items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-t-3xl sm:rounded-2xl w-full max-w-md">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea className="w-full p-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 resize-none bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400" rows={4} placeholder="Enter your warning message..." value={warningMessage} onChange={e => setWarningMessage(e.target.value)} />
              </div>

              <div className="flex space-x-3">
                <button onClick={() => setShowWarningModal(false)} className="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                  Cancel
                </button>
                <button onClick={sendWarning} disabled={!warningMessage.trim() || loading} className="flex-1 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center space-x-2">
