...
const selfService = {
    ...
    refreshNotifications: async (): Promise<{success: boolean, notifications?: Notification[], message?: string}> => {
        try {
            const response = await api.get(`/api/user/refresh-notifications`);
            if (response.data.success) {
                return { success: true, notifications: response.data.notifications };
            }
            return { success: false, message: "Failed to refresh notifications" };
        } catch (error: any) {
            return { success: false, message: error.message || "Failed to refresh notifications" };
        }
    }
}
